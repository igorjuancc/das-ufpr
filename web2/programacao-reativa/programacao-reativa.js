...
const obs2 = from(valores1).pipe(filter(valor => valor % 2 == 0), map(valor => valor / 10));
obs2.subscribe({
    next(valor) {
        console.log(valor);
    }
...
const obs3 = from(valores2).pipe(filter(valor => valor % 2 == 0), map(valor => valor / 10), take(2));
obs3.subscribe({
    next(valor) {
        console.log(valor);
    }
...
const obs4 = from(valores3).pipe(filter(valor => valor % 2 == 0), count());
obs4.subscribe({
    next(valor) {
        console.log(valor);
    }
...
const obs5 = from(valores4).pipe(count(valor => valor % 2 == 0));
obs5.subscribe({
    next(valor) {
        console.log(valor);
    }
...
const obs6 = from(valores5).pipe(max());
obs6.subscribe({
    next(valor) {
        console.log(valor);
    }
